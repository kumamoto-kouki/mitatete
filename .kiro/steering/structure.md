...
│   ├── tauri.conf.json
│   └── Cargo.toml
├── src/              # フロントエンド（HTML / CSS / JavaScript、フレームワークなし）
│   ├── index.html            # チャットUI
│   ├── character.html        # キャラクターウィンドウ（透明・常時最前面）
│   ├── js/                   # chat.js / character.js / principles.js / diary.js
│   ├── css/
│   └── assets/presets/       # プリセットキャラクター定義（*.json）
├── docs/             # コンセプトドキュメント
└── .kiro/            # ステアリング（steering/）・スペック（specs/）
```
