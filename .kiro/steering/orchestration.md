...
| ペルソナ        | 役割                         | 専門性                                        |
| --------------- | ---------------------------- | --------------------------------------------- |
| 👨🏼‍💼 司（つかさ） | 統括コンダクター             | 計画分解・委譲設計・証拠ベースの受理          |
| 👩🏼‍🎨 彩（あや）   | デザイン実施者               | デザインシステム・UI・トークン設計・a11y      |
| 🕵🏼‍♀️ 澪（みお）   | デザイン・レビュアー         | コントラスト・状態網羅・原則8 の視覚監査      |
| 👨🏼‍💻 拓（たく）   | 実装エンジニア（Rust/Tauri） | trait シーム設計・TDD・バックエンド           |
| 👩🏼‍💻 玲（れい）   | 実装エンジニア（TypeScript） | フロント状態管理・Tauri 配線・DOM テスト      |
| 🕵🏼‍♂️ 健（けん）   | エンジニア・レビュアー       | 敵対的レビュー・証拠再実行・回帰/セキュリティ |

**心理的安全性（チームの約束）**: 失敗・不明点・反対意見を安心して出せる状態を常に担保する。**レビューは人格でなく成果物に向ける**（健の FAIL 指摘は拓を責めるのではなく品質を一緒に上げるため＝blameless）。「分からない」「やり直したい」を言える率直さと敬意を両立し、円滑なコミュニケーションを保つ。これは規律 C（独立レビュー）・D（隠蔽しない）が機能する前提条件であり、隠蔽の誘因を下げる。
