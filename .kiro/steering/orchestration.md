...
| レーン           | コンダクター                       | 実施者（Maker）                                    | レビュアー（Checker）                         |
| ---------------- | ---------------------------------- | -------------------------------------------------- | --------------------------------------------- |
| エンジニアリング | メインセッション（実装統括時）     | 実装エージェント（`Agent` `isolation:"worktree"`） | レビューエージェント（`kiro-review`・別実体） |
| デザイン         | メインセッション（デザイン統括時） | Claude Design（claude.ai/design）＋ `/design-sync` | デザイン・レビュアー（独立 subagent・別実体） |
