...
- [x] 1. 基盤：モジュール・共通型・依存
- [x] 1.1 model-router の土台（依存・モジュール・共通型・エラー型）を用意する
...
- [x] 2. コア：システムプロンプト構築
- [x] 2.1 (P) PromptCharacter と原則値からシステムプロンプトを構築する
...
- [x] 3. コア：API キー秘匿管理
- [x] 3.1 (P) API キーの保存・照会と保護コマンドを実装する
...
- [x] 4. コア：provider クライアント
- [x] 4.1 Claude provider と共通 trait を実装する
...
- [x] 5. 統合：ModelRouter と Tauri コマンド
- [x] 5.1 ModelRouter（選択・ルーティング・履歴連携・エラー処理）を実装する
...
- [x] 5.2 Tauri コマンドを公開し lib.rs へ配線する
...
- [x] 6. 統合：フロントエンド UI
- [x] 6.1 (P) モデル選択・API キー設定 UI を実装する
...
- **依存・シーム再利用**：`reqwest`/`serde`/`keyring`/`tokio` は storage-manager で導入済みを再利用。**新規依存なし**。
- **実装時の調整（design からの差分・実態）**:
  - `async-trait` は**追加せず**、コードベース整合のため `#[allow(async_fn_in_trait)]` のネイティブ async fn ＋ enum/ジェネリック dispatch（`ModelRouter` が provider を match 分岐）を採用。
  - `HttpExecutor`（`execute -> Result<_, StorageError>`）は直接再利用せず、エラー型独立の `HttpClient`（`send -> Result<_, ModelError>`）シームを新設（同方針・ネットワーク非依存テスト）。
  - 履歴の成功時保存（要件6.1/6.2）は `ModelRouter` ではなく**フロント（chat.ts/main.ts）が担う**。`ModelRouter.generate` は呼び出し元 system を受ける汎用エントリに保ち、diary-engine が再利用できるようにした。7.1 の履歴系検証は chat.test.ts（成功時保存・失敗時非保存）＋ Rust（ApiKeyMissing 非送信・retry）で充足。
